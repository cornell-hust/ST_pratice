...
| M1-R02 | 用例不少于 30 条，采用等价类、边界值、场景法等方法 | CRON-UT-001—CRON-UT-036（另有5条专项自动化检查） | `testdata/cron_cases.json`、`tests/test_croniter.py` | `testcases/` 附录一 |
| M1-R03 | 覆盖表达式解析、迭代、匹配、范围和时区 | CRON-UT-005—CRON-UT-028 | `tests/test_croniter.py` | 41 条测试通过 |
| M1-R04 | 自动化比例达到课程要求（计划至少 80%） | 36 条清单用例全部自动化，另有5条专项检查 | `python -m pytest -q` | 当前基线测试集：41 passed |
...
| CRON-UT-025—032 | 连续迭代、范围枚举、匹配和时区场景 | `tests/test_croniter.py::test_scenarios`、`test_match_and_range` | 场景法、状态转换 | 已通过 |
...
| 最近一次执行命令和日期 | `PYTHONPATH=module1/subject python -m pytest -q -c module1/pytest.ini module1/tests`；2026-09-08 |
