...
| M1-R02 | 用例不少于 30 条，采用等价类、边界值、场景法等方法 | CRON-UT-001—011、013—025、033—035、037—052，共 43 条（满足不少于 30 条要求） | `testdata/cron_cases.json`、`tests/test_croniter.py` | `testcases/` 附录一 |
| M1-R03 | 覆盖表达式解析、迭代、匹配、范围和时区 | CRON-UT-005—CRON-UT-025、033—035、041—052 | `tests/test_croniter.py` | 43 条测试通过 |
| M1-R04 | 自动化比例达到课程要求（计划至少 80%） | 43 条清单用例全部自动化（原 5 条专项检查已并入账本） | `python -m pytest -q` | 当前测试集：43 passed |
...
| CRON-UT-013—024、034、035、042—044、051、052 | 分钟、小时、日期、月份、闰年、月末边界，夏令时（普通日/跳变日/回拨日）与返回值类型 | `tests/test_croniter.py::test_next_boundaries` | 边界值 | 已通过 |
...
| 项目 | 结果 |
|---|---|
| 总用例数 | 43 条清单用例（012 已删除、026—032 重复用例已去重、036 已删除；原 5 条专项检查并入 041—050，另新增 051—052 夏令时跳变/回拨用例） |
| 自动化用例数/比例 | 43 条 pytest 用例通过 |
| 通过/失败/跳过 | 43 / 0 / 0 |
