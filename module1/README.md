...
建议使用 Python 3.10 或更高版本，并在 `module1` 目录执行：
...
- 测试用例编号采用 `CRON-UT-001` 起始的连续编号；计划不少于 36 条，最终数量以清单和测试收集结果为准。
...
基线固定为上游 `croniter` 6.2.2，并在 `subject/croniter/SOURCE.md`（待创建）记录来源 URL、版本和校验信息。历史缺陷复现应先在基线状态执行并保存输出，再在独立修复分支修改源码，最后用同一回归用例验证。
